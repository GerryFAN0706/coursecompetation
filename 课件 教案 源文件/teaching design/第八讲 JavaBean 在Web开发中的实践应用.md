...
- 在上一讲中已使用Cursor生成JavaBean并进行规范性对比，具备初步的AI代码审查意识
- 本讲将进一步引导学生使用Cursor进行代码重构实践——将JSP中内嵌的业务逻辑提取为JavaBean，培养对AI重构建议的评估能力
...
第1步：教师展示一段典型的"意大利面条式"JSP代码（包含HTML、Scriptlet混杂的用户注册逻辑），分析其维护问题
     ↓
第2步：教师在Cursor中选中JSP代码，使用AI重构功能，提示"将此JSP中的业务逻辑提取为标准JavaBean，保留JSP仅用于页面展示"
     ↓
第3步：展示AI生成的重构方案（新的JavaBean类 + 简化后的JSP页面），引导学生讨论：AI提取的属性是否合理？验证逻辑应该放在Bean中还是Servlet中？作用域选择是否正确？
     ↓
第4步：学生在课堂练习中，将自己编写的学生成绩管理代码使用Cursor进行重构，提交重构前后的代码对比和AI建议评估报告
```
...
| 维度 | 传统课堂 | AI辅助课堂 |
|:---|:---|:---|
| 重构体验 | 手动重构耗时且容易出错，学生难以在有限课时内完成完整重构 | AI秒级生成重构方案，学生将精力集中在理解和评估重构结果上 |
| 问题发现深度 | 学生关注"代码能否运行"，较少思考结构设计合理性 | 通过评估AI重构建议，学生主动思考属性划分、逻辑归属、作用域选择等深层设计问题 |
| 实践转化率 | 理论讲解与实践应用之间存在较大鸿沟，学生知道规范但不会应用 | AI提供"从混乱到规范"的即时转化示范，降低实践门槛，提升知识迁移效率 |
...
### AI辅助评价
- 教师使用Cursor对学生提交的重构代码进行结构分析，快速检测是否存在JSP中残留的Scriptlet业务逻辑、JavaBean属性缺失getter/setter等问题
- 对比学生重构前后的代码，利用AI自动生成代码复杂度变化报告，量化评估重构效果（如代码行数减少比例、JSP中Java代码占比变化）
