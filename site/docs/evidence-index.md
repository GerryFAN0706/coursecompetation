---
title: 资料索引
description: 站点教学资料资产和命名规范索引页面。
lastUpdated: 2026-03-17
evidenceLevel: internal
anonSafe: true
---

# 资料索引

## 命名规范

- 截图：`Sxx_*.png`
- 文档：`Dxx_*.pdf` / `Dxx_*.md`
- 图表数据：`Cxx_*.json`

## 当前已接入资产

### 文档资产

- [D01_教学大纲.pdf](/assets/docs/D01_教学大纲.pdf)
- [D02_第十五讲教学设计.pdf](/assets/docs/D02_第十五讲教学设计.pdf)
- [D03_课堂视频选题与AI应用方案.pdf](/assets/docs/D03_课堂视频选题与AI应用方案.pdf)
- [D04_存在缺陷的示例工程设计方案.pdf](/assets/docs/D04_存在缺陷的示例工程设计方案.pdf)
- [D05_课堂实录逐分钟讲稿.pdf](/assets/docs/D05_课堂实录逐分钟讲稿.pdf)
- [D06_AI提示词与测试清单.pdf](/assets/docs/D06_AI提示词与测试清单.pdf)
- [D07_问题版工程README.pdf](/assets/docs/D07_问题版工程README.pdf)

### 图表数据

- [C01_代码质量对比.json](/assets/charts/C01_代码质量对比.json)
- [C02_代码规范性对比.json](/assets/charts/C02_代码规范性对比.json)
- [C03_SQL注入意识变化.json](/assets/charts/C03_SQL注入意识变化.json)
- [evidence-manifest.json](/data/evidence-manifest.json)

## 待补充（Phase 2）

- `S01-S12` 截图原始版与匿名版（目录结构与清单已建）
- `D07-D11` 扩展文档与示例工程双版本链接
- 更多图表数据集 (`C04-C12`)

## 说明

`/assets/evidence/original/` 与 `/assets/evidence/anonymized/` 已预置 `S01-S12` 文件位。
请将 `*_pending.png` 覆盖为真实截图，并同步执行匿名化检查。
