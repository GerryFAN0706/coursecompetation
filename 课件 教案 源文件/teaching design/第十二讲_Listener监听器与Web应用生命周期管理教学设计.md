...
- 在第九讲中使用ChatGPT进行MVC架构对比分析，在第十讲中使用Cursor辅助MVC项目调试，在第十一讲中使用Cursor进行Filter方案的设计安全审查，学生的AI使用能力已从"代码级调试"逐步升级为"方案级评估"
- **当前AI能力边界**：学生已能使用AI进行代码调试（L10）和配置方案审查（L11），但尚未尝试过使用AI进行跨知识点的对比分析和场景选型决策；学生能评判"这段代码是否正确"，但缺乏借助AI进行"这个场景该用A技术还是B技术"的选型分析经验
- **AI应用层次递进**：本讲是课程AI应用的第四个层次——从代码生成（L7-L8）→架构分析（L9）→项目调试（L10）→方案审查（L11）→知识对比与场景分析（L12），学生需要学会将AI作为"知识拓展工具"而非仅仅是"编码辅助工具"
...
第1步：教师讲解完三类Listener的代码实现后，提出问题："如果你的项目需要统计在线人数、记录请求耗时、在启动时加载配置，分别该用哪种Listener？"
     ↓
第2步：学生在ChatGPT/Claude中提问，获取AI的场景对比分析
     ↓
第3步：学生阅读AI的场景分析回答，逐条对照本讲的三个实战案例验证AI分析的准确性
     ↓
第4步：学生标注AI回答中的正确点与可能不准确之处，在课堂讨论环节分享验证结果
```

**示例提示词**：

> **场景对比分析提示词**："请对比Java Servlet规范中ServletContextListener、HttpSessionListener、ServletRequestListener三种监听器的适用场景。要求：(1) 分别说明每种Listener监听的事件类型和触发时机；(2) 各举3个实际项目中的应用场景；(3) 说明它们与Filter的区别——哪些需求应该用Filter处理而不是Listener？(4) 在并发场景下各自需要注意什么安全问题？"
>
> **场景选型决策提示词**："我正在开发一个Java Web图书管理系统，现在有以下需求，请帮我判断每个需求应该使用哪种Listener（或是否应该用Filter替代），并说明理由：(1) 应用启动时从数据库预加载图书分类数据到内存；(2) 统计当前在线用户数量；(3) 记录每个请求的处理耗时；(4) 当用户登录成功后在Session中设置用户信息时记录审计日志；(5) 在用户Session超时时清理该用户的临时文件。"
...
| 指标 | 目标值 | 测量方式 |
|:---|:---|:---|
| Listener选型正确率 | ≥85%学生能在给定场景中正确选择Listener类型 | 课堂选型练习 |
| AI验证报告完成率 | ≥90%学生提交包含验证分析的报告 | 课堂作业收集 |
| AI偏差识别能力 | ≥60%学生能在AI回答中发现至少1处不准确或遗漏之处 | 验证报告评分 |
| Filter vs Listener辨析准确率 | ≥75%学生能正确区分应使用Filter还是Listener的场景 | 课后测试 |
...
### AI辅助评价
- **验证能力分级评估**：收集学生在AI对比分析环节的验证报告，按验证深度分级——L1（仅复述AI回答内容，未进行验证）、L2（能对照课堂案例逐条验证AI分析的准确性）、L3（能发现AI分析中的遗漏或偏差，如AI未提及并发安全问题、错误地将Filter场景归为Listener），目标为≥50%学生达到L2以上
- **跨知识点关联能力评估**：评估学生是否在AI对比分析中主动将Listener与上一讲Filter进行横向比较，判断学生是否形成了"容器级组件知识体系"的整体认知（Filter负责请求处理链 vs Listener负责生命周期事件感知）
- **场景选型实效评估**：在课后作业中设置"场景选型"题（给出3-4个实际需求场景，要求学生选择合适的Listener类型并解释理由），对比经过AI场景分析训练的班级与未经训练班级的正确率差异，量化AI辅助场景分析对选型决策能力的提升效果
